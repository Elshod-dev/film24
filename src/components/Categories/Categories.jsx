...
function Categories() {
  ...
  const deleteCategory = (e) => {
    ...
    setErrMessage(false);
    axios
      .delete(API_URL + "/categories/" + id, config)
      .then((res) => {
        console.log(res);
        setSuccessMessage(true);
        setSuccessMessageText("Category successfully deleted!");
  ...
  const createHandle = (e) => {
    const checkInputValidity = (inputWord, data) => {
      return data.some(
        (item) => item.title.toLowerCase() === inputWord.toLowerCase()
      );
    };
    e.preventDefault();
    ...
    if (checkInputValidity(categoryValue.current.value, categories)) {
      setSuccessMessage(false);
      setErrMessage(true);
      ...
      axios
        .post(
          API_URL + "/categories",
          { title: categoryValue.current.value },
          config
        )
        .then((res) => {
          setSuccessMessage(true);
          setIsRefresh(!isRefresh);
          setSuccessMessageText("Category created successfully!");
        ...
        <div className={styles.categories}>
          ...
          <div className={styles.categories__body}>
            {categories.map((category) => (
              <div key={category._id} className={styles.categories__infos}>
